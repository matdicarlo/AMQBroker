# Scalable Partitioned Consumers with Federation (using Connection Router and Hash Module)

This project implements a horizontally scalable and partitioned MQTT architecture using AMQ Broker (ActiveMQ Artemis) and Camel Quarkus. It demonstrates how to federate brokers and route MQTT clients to specific cluster nodes using a connection router and a consistent hash modulo policy, ensuring sticky session state. 

*Note: While this architecture provides horizontal scaling and recoverable fault tolerance, it does not provide High Availability (HA). It currently lacks active/passive state replication and uses hardcoded client endpoints. If a broker node fails, clients pinned to that node must wait for it to recover to resume their specific sessions.*
